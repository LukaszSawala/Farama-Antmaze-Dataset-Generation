...
def calculate_maze_stats(dataset):
    """Calculates maze-specific stats (success rate and avg velocity)."""
    successes = 0
    velocity_sum = 0.0

    for ep in dataset:
        if "success" in ep.infos:
            if np.any(ep.infos["success"]):
                successes += 1
        else:
            achieved = ep.observations["achieved_goal"]
            desired = ep.observations["desired_goal"]
            distances = np.linalg.norm(achieved - desired, axis=1)
            
            # AntMaze success threshold is standard at 0.5 meters
            if np.any(distances <= 0.5):
                successes += 1
        coords = ep.observations["achieved_goal"]
        velocity_sum += np.linalg.norm(coords[1:] - coords[:-1], axis=1).sum()

    success_rate = successes / dataset.total_episodes
    avg_velocity = velocity_sum / dataset.total_steps
    
    return success_rate, avg_velocity
